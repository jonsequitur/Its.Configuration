...
namespace Its.Configuration
{
    ...
    public static class CompositionExtensions
    {
        ...
        internal static IEnumerable<ConfigurationDescription> FindImports(this DirectoryInfo directory)
        {
            return directory
                .GetFiles("*.dll")
        ...
        internal static IEnumerable<ConfigurationDescription> FindImports(this IEnumerable<Assembly> assemblies)
        {
        ...
        internal static IEnumerable<ConfigurationDescription> FindImports(this IEnumerable<Type> types)
        {
            return types
                .Select(t =>
                        new
                        {
                            Type = t,
                            ImportingMembers = t.ImportingMembers()
                        })
                .SelectMany(t =>
                            t.ImportingMembers.Select(m => new
                            {
                                t.Type,
                                Member = m
                            }))
                ...
                .Select(t =>
                {
                    var returnType = t.Member.ReturnType();
                    return new ConfigurationDescription
                    {
                        Id = returnType.Key() + ":" + t.Import.ContractName,
                        ExpectedType = returnType,
                        DeclaringType = t.Type,
                        Name = t.Import.ContractName,
                        AllowDefault = t.Import.AllowDefault
                    };
                }
                );
        }

        internal static IEnumerable<ConfigurationDescription> FindImports(this ComposablePartCatalog catalog)
        {
            if (catalog == null)
            {
            ...
            var assembliesCatalog = catalog as AssembliesCatalog;
            if (assembliesCatalog != null)
            {
                foreach (var import in  assembliesCatalog.Assemblies().SelectMany(a => a.GetExportedTypes()).FindImports())
                {
                    yield return import;
                }
            }
            ...
            {
                foreach (var importDefinition in part.ImportDefinitions.OfType<ContractBasedImportDefinition>())
                {
                    var importingLazyMember = importDefinition.GetType().GetProperty("ImportingLazyMember");
                    if (importingLazyMember != null)
                    {
                        var importingLazyDefinition = (LazyMemberInfo) importingLazyMember
                                                                           .GetValue(importDefinition, BindingFlags.GetProperty, null, null, null);

                        var returnType = importingLazyDefinition.GetAccessors().First().ReturnType();

                        yield return new ConfigurationDescription
                        {
                            Id = importDefinition.Key(),
                            ExpectedType = returnType
                        };
                    }
                }
        ...
        internal static IEnumerable<UpdateableConfigExport> ResolveExports(
            this ImportDefinition definition,
            GetConfigurationValue getConfigValueString,
            Dictionary<string, Func<string, object>> typeConverters)
        {
            object configValue;
            try
            {
                configValue = getConfigValueString(definition.ContractName);
            }
            catch (Exception exception)
            {
                ...
                throw new ConfigurationErrorsException(
                    string.Format("An error occurred while looking up a configuration value named '{0}'.", definition.ContractName),
                    exception);
            }

            if (configValue != null)
            {
                var contractBasedImportDef = definition as ContractBasedImportDefinition;

                if (contractBasedImportDef != null)
                {
                    Func<object, object> convert = value => GetConverter(
                        typeConverters,
                        value,
                        contractBasedImportDef.RequiredTypeIdentity,
                        definition.ContractName)();

                    return new[]
                    {
                        new UpdateableConfigExport(definition.ContractName, configValue, convert, typeConverters)
                    };
                }
            }

            return Enumerable.Empty<UpdateableConfigExport>();
        }

        internal static Func<object> GetConverter(
            Dictionary<string, Func<string, object>> typeConverters,
            object configValue,
            string typeIdentity,
            string contractName)
        {
            Func<string, object> customConverter;
            Func<object> exportedValueGetter;
            if (typeConverters.TryGetValue(typeIdentity, out customConverter))
            {
                exportedValueGetter = () => customConverter((string) configValue);
            }
            else
            {
                var targetType = GetTargetType(typeIdentity);

                if (targetType == null)
                {
                    throw new TypeAccessException(
                        string.Format(
                            "Could not find type \"{0}\" when resolving imports{1}. Try specifying a conversion using ConfigurationValueExportProvider.RegisterConversion.",
                            typeIdentity,
                            contractName == null ? "" : string.Format(" for contract \"{0}\"", contractName)));
                }

                if (targetType.IsEnum)
                {
                    exportedValueGetter = () => Enum.Parse(targetType, configValue.ToString());
                }
                else if (targetType.IsGenericType && targetType.GetGenericArguments().First().IsEnum)
                {
                    exportedValueGetter = () => Enum.Parse(targetType.GetGenericArguments().First(), configValue.ToString());
                }
                else
                {
                    exportedValueGetter = () => Convert.ChangeType(configValue, targetType);
                }
            }

            // TODO: (GetConverter) get the member name
            return exportedValueGetter.WithInformativeException(contractName, typeIdentity, null);
        }

        private static Func<object> WithInformativeException(
            ...
            string memberName)
        {
            return () =>
            {
                ...
                {
                    throw new CompositionException(String.Format("Import '{0}' on member {1} ({2}) could not be satisfied due to a {3}",
                                                                 contractName,
                                                                 memberName,
                                                                 GetTargetType(typeIdentity).FullName,
        ...
        private static Type GetTargetType(string typeIdentity)
        {
            var targetType = Type.GetType(typeIdentity);

            return targetType ?? discoveredTypes.GetOrAdd(
                typeIdentity,
                ti =>
                {
                    MatchCollection parseResult = typeIdentifierParser.Matches(ti);
                    if (parseResult.Count > 0)
                    {
                        // TODO: (GetTargetType) make this more robust / flexible
                        var genericType = GetTargetType(parseResult[0].Groups[1].Value + "`1");
                        var genericArg = GetTargetType(parseResult[0].Groups[2].Value);
                        return genericType.MakeGenericType(genericArg);
                    }
                    ...
                    return AppDomain.CurrentDomain
                                    ...
                                    .FirstOrDefault(t => t.FullName == ti);
                });
        }

        private static readonly Regex typeIdentifierParser = new Regex(
            "^([^(]+)\\((.*)\\)$",
